...
from config import *
from utils import *
from networks.FCNWideResNet50 import FCNWideResNet50Decoupled
...
def train(train_loader, net, ce_criterion, tl_criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        # Updating loss meter.
        train_loss.append((loss_tl.data.item() + loss_ce.data.item()))
...
if __name__ == '__main__':
    ...
    if args.operation == 'Train':
        ...
        if args.model == 'WideResNet':
            model = FCNWideResNet50Decoupled(train_dataset.num_classes, pretrained=True, skip=True, classif=True)
        else:
        ...
        # network
        if args.model == 'WideResNet':
            model = FCNWideResNet50Decoupled(test_dataset.num_classes, pretrained=True, classif=False)
        else:
            raise NotImplementedError("Network " + args.model + " not implemented")
