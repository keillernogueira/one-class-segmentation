...
from dataloaders.data_utils import create_distrib, create_or_load_statistics, split_train_test, \
    normalize_images, data_augmentation


class DataLoaderTreeSubSet(data.Dataset):
    ...
    def load_images(self):
        images = []
        masks = []
        temp_image = img_as_float(imageio.imread(os.path.join(self.dataset_input_path, 'ARV' + self.images + '.tif')))
        temp_mask = imageio.imread(os.path.join(self.dataset_input_path, 'ARV' + self.images + '_mask.tif')).astype(int)
    ...
    def make_dataset(self):
        all_distrib, gen_classes = create_distrib(self.labels, self.crop_size, self.stride_size,
                                                  self.num_classes, self.dataset, return_all=True)
        train_distrib, test_distrib = split_train_test(all_distrib, limit=4000)

        if self.mode == 'Train':
            return train_distrib, gen_classes
        else:
            return test_distrib, gen_classes
