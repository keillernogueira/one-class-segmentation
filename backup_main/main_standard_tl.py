...
# knn
def test_per_patch(knn_dataloader, test_loader, net, epoch, k=10):
...
def train(train_loader, net, optimizer, epoch, margin):
    ...
    for i, data in enumerate(train_loader):
        ...
        if (i + 1) % DISPLAY_STEP == 0:
            acc = 0.0  # calc_accuracy_triples(a, p, n)
...
if __name__ == '__main__':
    ...
    # specific parameters
    parser.add_argument('--k', type=int, default=3, help='k for the kNN')
    ...
    if args.operation == 'Train':
        ...
        for epoch in range(curr_epoch, args.epoch_num + 1):
            train(train_dataloader, model, optimizer, epoch, args.margin)
            if epoch % VAL_INTERVAL == 0:
                # Computing test.
                acc, nacc = test_per_patch(knn_dataloader, test_dataloader, model, epoch, args.k)
        ...
        test_per_patch(knn_dataloader, test_dataloader, model, epoch, args.k)
    else:
